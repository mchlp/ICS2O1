...
def createPairList(wordLen, complexity, toUseDict):
    ...
    def createConnectionList(matchList, complexity):
    
        #CREATE LIST OF PAIRS

        def findConnections(matchList, path, cmplx, oldList):
            curWord = path[-1]
            if cmplx > 0:
                #print("   "*cmplx, "PROCESSING", curWord, "PATH", path, "COMPX", cmplx)
                #print("     ", matchList[curWord])
                oldList = oldList + matchList[curWord]                
                for word in matchList[curWord]:
                    #print("       goto:", word)
                    findConnections(matchList, path+[word], cmplx-1, oldList)
            else:
                #print("NO MORE COMPLEXITY", path)
                for word in matchList[curWord]:
                    global pairList
                    pairList.append(path + [word])
                    #print(path, word)
        ...
        oneDash = (totalWords*complexity)//(scrnWidth-2)
        counter = 0
        keylist = list(wordList.keys())
        pathHead = []
        pathHead.append([])
        for word in keylist:
            for complexword in matchList[word]:
                pathHead[0].append([word, complexword])
                pairList.append([word, complexword])
                print([word, complexword])

        for complex in range(1, complexity):
            pathHead.append([])
            for path in pathHead[complex-1]:
                lastword = path[-1]
                for complexword in matchList[lastword]:
                    pathHead[complex].append(path + [complexword])
                    pairList.append(path + [complexword])
                    print(path + [complexword])
                              
#            for rootWord in keylist:
#                counter += 1
#                if counter > oneDash:
#                    #print(printColour("-"), end="")
#                    counter = 0
...
#                findConnections(matchList, path, cmplx, [])
    ...
    print(printColour(" |"), end="")    
    createConnectionList(matchList, complexity)
    print(printColour("|"))
...
start = time.time()
createPairList(wordLen, complexity, toUseDict)
end = time.time()
